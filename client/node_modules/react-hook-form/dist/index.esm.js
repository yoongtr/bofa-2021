...
const focusFieldBy = (fields, callback, fieldsNames) => {
    for (const key of fieldsNames || Object.keys(fields)) {
        ...
        if (field) {
            const { _f } = field, current = __rest(field, ["_f"]);
...
var getFieldValueAs = (value, { valueAsNumber, valueAsDate, setValueAs }, shouldReturnAsValue) => shouldReturnAsValue
    ? valueAsNumber
        ? value === ''
            ? NaN
            : +value
...
const getFieldsValues = (fieldsRef, defaultValuesRef, shouldReturnAsValue, output = {}) => {
    for (const name in fieldsRef.current) {
        const field = fieldsRef.current[name];
        if (field) {
            const { _f } = field, current = __rest(field, ["_f"]);
            set(output, name, _f && !_f.ref.disabled
...
var getRadioValue = (options) => Array.isArray(options)
    ? options.reduce((previous, option) => option && option.checked
        ? {
            isValid: true,
            value: option.value,
        }
...
var getFieldArrayParentName = (name) => name.substring(0, name.search(/.\d/)) || name;

var isNameInFieldArray = (names, name) => [...names].some((current) => getFieldArrayParentName(name) === current);
...
const getPath = (rootPath, values, paths = []) => {
    for (const property in values) {
        isPrimitive(values[property])
            ? paths.push(`${rootPath}.${property}`)
            : getPath(`${rootPath}.${property}`, values[property], paths);
    }
    return paths;
};
...
function useForm({ mode = VALIDATION_MODE.onSubmit, reValidateMode = VALIDATION_MODE.onChange, resolver, context, defaultValues = {}, shouldFocusError = true, criteriaMode, } = {}) {
    ...
    const fieldsNamesRef = useRef(new Set());
    const formStateSubjectRef = useRef(new Subject());
    const watchSubjectRef = useRef(new Subject());
    const controllerSubjectRef = useRef(new Subject());
    const fieldArraySubjectRef = useRef(new Subject());
    const fieldArrayUpdatedValuesRef = useRef({});
    ...
    const shouldRenderBaseOnError = useCallback((name, error, shouldRender = false, state = {}, isValid) => {
        ...
        if ((shouldReRender && !isNullOrUndefined(shouldRender)) ||
            !isEmptyObject(state)) {
            formStateSubjectRef.current.next(Object.assign(Object.assign({}, state), { isValid: resolverRef.current ? isValid : getIsValid(), errors: formStateRef.current.errors }));
        }
    ...
    const setFieldValue = useCallback((name, rawValue) => {
        const { _f } = get(fieldsRef.current, name);
        if (_f) {
            const { ref, refs } = _f;
            ...
            _f.value = rawValue;
            if (isRadioInput(ref)) {
                (refs || []).forEach((radioRef) => (radioRef.checked = radioRef.value === value));
            }
                ...
                refs.length > 1
                    ? refs.forEach((checkboxRef) => (checkboxRef.checked = Array.isArray(value)
    ...
    const getFormIsDirty = useCallback((name, data) => {
    ...
    const updateAndGetDirtyState = useCallback((name, shouldRender = true) => {
        if (readFormStateRef.current.isDirty ||
            ...
            const isFieldDirty = !deepEqual(get(defaultValuesRef.current, name), getFieldValue(get(fieldsRef.current, name), true));
    ...
    const executeSchemaOrResolverValidation = useCallback(async (names, currentNames = []) => {
        const { errors } = await resolverRef.current(getFieldsValues(fieldsRef, defaultValuesRef, true), contextRef.current, {
            criteriaMode,
            names: currentNames,
    ...
    const setInternalValue = useCallback((name, value, options) => {
    ...
    const handleChange = useCallback(async ({ type, target, target: { value, type: inputType } }) => {
    ...
    const updateIsValid = useCallback(async (values = {}) => {
        if (resolver) {
            const { errors } = await resolverRef.current(Object.assign(Object.assign({}, getFieldsValues(fieldsRef, defaultValuesRef, true)), values), contextRef.current, {
                criteriaMode,
                fields: getFields(fieldsNamesRef.current, fieldsRef.current),
            });
    ...
    const watchInternal = useCallback((fieldNames, defaultValue, isGlobal) => {
    ...
    const unregister = (name, options = {}) => {
        ...
        formStateSubjectRef.current.next(Object.assign(Object.assign(Object.assign({}, formStateRef.current), (!options.keepDirty ? {} : { isDirty: getFormIsDirty() })), (resolver ? {} : { isValid: getIsValid() })));
        if (!options.keepIsValid) {
            updateIsValid();
        }
    };
    ...
    const resetFromState = ({ keepErrors, keepDirty, keepIsSubmitted, keepTouched, keepIsValid, keepSubmitCount, }) => {
        if (!keepIsValid) {
...
const FormProvider = (_a) => {
    ...
    return (createElement(FormContext.Provider, { value: props }, children));
};
...
const useFieldArray = ({ control, name, keyName = 'id', }) => {
    ...
    const [fields, setFields] = useState(mapIds(get(fieldArrayDefaultValuesRef.current, getFieldArrayParentName(name))
    ...
    const getFocusDetail = (index, options) => {
        if (options) {
    ...
    const resetFields = (index) => (Array.isArray(index) ? index : [index]).forEach((currentIndex) => set(fieldsRef.current, `${name}${currentIndex ? `.${currentIndex}` : ''}`, []));
    const setFieldsAndNotify = (fieldsValues) => {
    ...
    const prepend$1 = (value, options) => {
        const updatedFieldArrayValues = prepend(getCurrentFieldsValues(), Array.isArray(value) ? value : [value]);
    ...
    const insert$1 = (index, value, options) => {
        const fieldValues = getCurrentFieldsValues();
        const updatedFieldArrayValues = insert(fieldValues, index, Array.isArray(value) ? value : [value]);
        setFieldsAndNotify(updatedFieldArrayValues);
...
function useFormState({ control, } = {}) {
    ...
    const [formState, updateFormState] = useState(formStateRef.current);
    const readFormState = useRef(cloneObject(readFormStateRef.current));
    useEffect(() => {
        const formStateSubscription = formStateSubjectRef.current.subscribe({
            next: (formState) => {
...
var getControllerValue = (event) => isPrimitive(event) ||
...
function useController({ name, rules, defaultValue, control, }) {
    ...
    const getInitialValue = () => (get(fieldsRef.current, name) &&
...
function useWatch({ control, name, defaultValue, }) {
    const methods = useFormContext();
    const { watchInternal, defaultValuesRef, watchSubjectRef } = control || methods.control;
    const [value, updateValue] = useState(isUndefined(defaultValue)
        ? Array.isArray(name)
...
export { Controller, FormProvider, appendErrors, get, set, transformToNestObject, useController, useFieldArray, useForm, useFormContext, useFormState, useWatch };
